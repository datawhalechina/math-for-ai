...
**定义2.4（转置）**：对于矩阵 $\boldsymbol{A}\in \R^{m\times n}$，满足 $b_{ij}=a_{ji}$ 的矩阵 $\boldsymbol{B}\in \R^{n\times m}$ 被称作 $\boldsymbol{A}$ 的转置。我们记 $\boldsymbol{B}=\boldsymbol{A}^{\top}$
