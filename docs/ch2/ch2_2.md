...
**定义2.1（矩阵）**：对于 $m,n\in \Z$，一个形状为 $(m,n)$ 的实矩阵 $\boldsymbol{A}$ 是一个关于元素 $a_{ij}$ 的 $m\times n$ 元组，其中 $i=1,2,\dots,m, j=1,2,\dots,n$，按照 $m$ 行 $n$ 列的方式进行排布。
...
按照惯例，形状为 $(1, n)$ 的矩阵成为行向量，形状为 $(1, m)$ 的矩阵称为列向量。
